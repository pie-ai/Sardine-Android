...
/**
 * Describes a resource on a remote server. This could be a directory or an
 * actual file.
 *
 * @author jonstevens
...
public class DavResource {
	...
	/**
	 * The default content-type if {@link Getcontenttype} is not set in the
	 * {@link Multistatus} response.
	 */
	public static final String DEFAULT_CONTENT_TYPE = "application/octetstream";

	/**
	 * The default content-lenght if {@link Getcontentlength} is not set in the
	 * {@link Multistatus} response.
	 */
	 ...
	 */
	protected DavResource(String href, Date creation, Date modified,
			String contentType, Long contentLength, String etag,
	...
	/**
	 * Converts the given {@link Response} to a
	 * {@link de.aflx.sardine.DavResource}.
	 *
	 * @param response
	...
	private String getModifiedDate(Response response) {
		...
		/*
		 * if (list.isEmpty()) { return null; } for(Propstat propstat: list) {
		 * Getlastmodified glm = propstat.getProp().getGetlastmodified(); if
		 * ((glm != null) && (glm.getContent().size() == 1)) { return
	...
	private String getCreationDate(Response response) {
		...
		/*
		 * List<Propstat> list = response.getPropstat(); if (list.isEmpty()) {
		 * return null; } for(Propstat propstat: list) { Creationdate gcd =
		 * propstat.getProp().getCreationdate(); if ((gcd != null) &&
		 * (gcd.getContent().size() == 1)) { return gcd.getContent().get(0); } }
		 * return null;
		 */
	}

	/**
	 * Retrieves the content-type from prop or set it to
	 * {@link #DEFAULT_CONTENT_TYPE}. If isDirectory always set the content-type
	 * to {@link #HTTPD_UNIX_DIRECTORY_CONTENT_TYPE}.
	 *
	 * @param response
	 *            The response complex type of the multistatus
	...
	private String getContentType(Response response) {
		...
		// If there is no content type set in the properties, we will parse the
		// href
		if (null == type) {
		 ...
		 * List<Propstat> list = response.getPropstat(); if (list.isEmpty()) {
		 * return null; } for(Propstat propstat: list) { Resourcetype
		 * resourcetype = propstat.getProp().getResourcetype(); if
		 * ((resourcetype != null) && (resourcetype.getCollection() != null)) {
		 * // Need to correct the contentType to identify as a directory. return
		 * HTTPD_UNIX_DIRECTORY_CONTENT_TYPE; } else { Getcontenttype gtt =
		 * propstat.getProp().getGetcontenttype(); if ((gtt != null) &&
		 * (gtt.getContent().size() == 1)) { return gtt.getContent().get(0); } }
	...
	private long getContentLength(Response response) {
		...
		/*
		 * List<Propstat> list = response.getPropstat(); if (list.isEmpty()) {
		 * return DEFAULT_CONTENT_LENGTH; } for(Propstat propstat: list) {
		 * Getcontentlength gcl = propstat.getProp().getGetcontentlength(); if
		 * ((gcl != null) && (gcl.getContent().size() == 1)) { try { return
		 * Long.parseLong(gcl.getContent().get(0)); } catch
		 * (NumberFormatException e) {
		 * log.warn(String.format("Failed to parse content length %s",
		 * gcl.getContent().get(0))); } } } return DEFAULT_CONTENT_LENGTH;
		 */
	}
	...
	private String getEtag(Response response) {
		...
		/*
		 * List<Propstat> list = response.getPropstat(); if (list.isEmpty()) {
		 * return null; } for(Propstat propstat: list) { Getetag etag =
		 * propstat.getProp().getGetetag(); if ((etag != null) &&
		 * (etag.getContent().size() == 1)) { return etag.getContent().get(0); }
		 * } return null;
		 */
	}

	/**
	 * Creates a simple complex Map from the given custom properties of a
	 * response. This implementation does take namespaces into account.
	 *
	 * @param response
	 *            The response complex type of the multistatus
	...
	private Map<QName, String> getCustomProps(Response response) {
		...
		/*
		 * List<Propstat> list = response.getPropstat(); if (list.isEmpty()) {
		 * return null; } Map<QName, String> customPropsMap = new HashMap<QName,
		 * String>(); for(Propstat propstat: list) { List<Element> props =
		 * propstat.getProp().getAny(); for (Element element : props) { String
		 * namespace = element.getNamespaceURI(); if (namespace == null) {
		 * customPropsMap.put(new QName(SardineUtil.DEFAULT_NAMESPACE_URI,
		 * element.getLocalName(), SardineUtil.DEFAULT_NAMESPACE_PREFIX),
		 * element.getTextContent()); } else { if (element.getPrefix() == null)
		 * { customPropsMap.put(new QName(element.getNamespaceURI(),
		 * element.getLocalName()), element.getTextContent()); } else {
		 * customPropsMap.put(new QName(element.getNamespaceURI(),
	 ...
	 * <code>httpd/unix-directory</code> is a directory.
	 *
	...
	/**
	 * @return Additional metadata. This implementation does not take namespaces
	 *         into account.
	 */
	 ...
	 * Last path component.
	 *
	 * @return The name of the resource URI decoded. An empty string if this
	 *         resource denotes a directory.
	 * @see #getHref()
	 */
